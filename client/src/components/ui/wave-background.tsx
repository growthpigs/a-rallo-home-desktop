...
export function Waves({
    ...
    const setLines = () => {
        ...
        // Medium spacing for balanced scale and detail
        const xGap = 15  // Medium horizontal spacing for balanced waves
        const yGap = 15  // Medium vertical spacing for balanced waves
        ...
        for (let i = 0; i < totalLines; i++) {
            ...
            path.setAttribute('opacity', '0.15')  // 15% opacity for subtle lines
    ...
    const movePoints = (time: number) => {
        ...
        lines.forEach((points) => {
            points.forEach((p: Point) => {
                // Wave movement - balanced between smooth and detailed
                const move = noise(
                    (p.x + time * 0.005) * 0.0015,  // Medium frequency for balanced undulation
                    (p.y + time * 0.002) * 0.001    // Medium frequency for visible waves
                ) * 6  // Moderate amplitude for noticeable but smooth waves

                p.wave.x = Math.cos(move) * 15  // Balanced horizontal movement
                p.wave.y = Math.sin(move) * 12   // Balanced vertical movement
